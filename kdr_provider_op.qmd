...
[This table lists the top ten survey questions, sorted in order of priority, that drive the respondents' top box score for the Institutional Measure: ***"`r ltr[,1]`"***]{.important}
...
total%>%
  ...
  # tab_footnote(
  #   footnote = ltr[,4] Received dates from  "
  # )
...
-   ***`r ltr[,4]`***  Press Ganey Patient Satisfaction surveys 

-   Received dates from ***`r min(data$newdate)`*** to ***`r max(data$newdate)`***.

-   The [percentile rank]{.underline} is an internal rank that compares all physicians within the service line. For example, a percentile rank of 90 means the score is better than 90% of all physicians' scores within Moffitt.
