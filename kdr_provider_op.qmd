...
-   The percentile rank is an internal rank that compares all physicians within the service line. For example, a percentile rank of 90 means the score is in the top 10 percent of all physicians.

-   The correlation measures the strength of the linear association between the response to the institutional measure and the question of interest. The survey count of `r ltr[,4]` *`r (if(ltr[,4] > 29 ) {print("meets")} else if (ltr[,4] < 30) {print("fails to meet")})`* the minimum number of surveys required to come to a *statistically sound conclusion.*
